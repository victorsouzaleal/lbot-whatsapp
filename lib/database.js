...
module.exports = {
    ...
    verificarDonoAtual : async(id_usuario)=>{
        ...
        if(!usuario){
            await db.usuarios.asyncUpdate({tipo: "dono"}, {$set:{tipo: "comum",  max_comandos_dia : limite_diario.qtd}}, {multi: true})
    ...
    ultrapassouLimite: async(id_usuario)=>{
        let usuario =  await db.usuarios.asyncFindOne({id_usuario : id_usuario})
        var tiposSemLimite = ["dono", "vip"]
        if(tiposSemLimite.includes(usuario.tipo)) return false

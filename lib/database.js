...
module.exports = {
    ...
    verificarCamposUsuarios : async()=>{
        var usuarios = await db.usuarios.asyncFind({})
        for(var u of usuarios){
            //ADICIONAR FUTURAS ALTERAÇÕES
        }
    },
    ...
    verificarCamposGrupos : async()=>{
        let grupos = await db.grupos.asyncFind({})
        for(g of grupos){
            if(!g.bemvindo) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"bemvindo.status": false, "bemvindo.msg": ""}})
            if(!g.antifake) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"antifake.status": false, "antifake.ddi_liberados": []}})
            if(!g.antilink) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"antilink.status": false, "antilink.filtros.youtube": false, "antilink.filtros.whatsapp": false, "antilink.filtros.facebook": false, "antilink.filtros.twitter": false}})
            if(!g.antiflood) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{antiflood : false}})
            if(!g.voteban)  await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"voteban.status" : false, "voteban.max": 5, "voteban.usuario": "", "voteban.votos": 0, "voteban.votou": []}})
            if(!g.contador) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"contador.status" : false, "contador.inicio": ""}})
            if(!g.enquete) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"enquete.status" : false, "enquete.pergunta": "", "enquete.opcoes": []}})
            if(!g.block_cmds) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{block_cmds : []}})
            if(!g.lista_negra) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{lista_negra : []}})
            if(!g.antitrava) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{"antitrava.status": false, "antitrava.max_caracteres": 0}})
            if(!g.antiporno) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{antiporno : false}})
            if(!g.autosticker) await db.grupos.asyncUpdate({id_grupo : g.id_grupo}, {$set:{autosticker : false}})
        }
    },

...
export const checagemMensagem = async (c, mensagemInfoCompleta) => {
    try {
        ...
        let registrado = await usuarios.verificarRegistro(sender)
        if(!registrado) {
            if(isOwner) {
                ...
                //SE NÃO FOR MENSAGEM DE GRUPO, A MENSAGEM NÃO TIVER UM COMANDO, PV ESTIVER LIBERADO E O BOT NÃO ESTIVER NO NUMERO DO DONO, ENVIE UM GUIA
                if(!isGroupMsg && !comandoExiste && botInfoJSON.pvliberado && ownerNumber != botNumber){
                    await socket.sendText(c, chatId, criarTexto(msgs_texto.geral.usuario_novo, nome_bot, username))
                }
            }
        } else {

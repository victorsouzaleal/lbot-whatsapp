...
import {botInfoUpdate, botLimitarComando, botInfo, botVerificarExpiracaoLimite,botLimitarMensagensPv, botAlterarNumeroDono} from '../db-modulos/bot.js'
...
export const checagemMensagem = async (c, messageTranslated) => {
    try {
        const lista_comandos = listarComandos(), msgs_texto = obterMensagensTexto(), botInfoJSON = botInfo(), {prefixo, numero_dono, nome_bot} = botInfoJSON
        const {sender, isGroupMsg, type, caption, id, chatId, body, username, participant, messageId} = messageTranslated

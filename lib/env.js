...
module.exports = {
    ...
    verificarEnv:()=>{
        let resposta = {
            ...
            twitter :{
              resposta: (process.env.twitter_consumer_key.trim() == "??????" || process.env.twitter_consumer_secret.trim() == "??????" || process.env.twitter_access_token.trim() == "??????" || process.env.twitter_access_token_secret.trim() == "??????" )
               ? "A API do Twitter ainda não foi configurada corretamente" : "✓ API Twitter Configurada.",
              cor_resposta: (process.env.twitter_consumer_key.trim() == "??????" || process.env.twitter_consumer_secret.trim() == "??????" || process.env.twitter_access_token.trim() == "??????" || process.env.twitter_access_token_secret.trim() == "??????" )
              ? "red" : false
            },
            acrcloud :{

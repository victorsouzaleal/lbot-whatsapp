...
module.exports = {
    ...
    verificarCamposEnv : async ()=>{
        var env = fs.readFileSync(path.resolve('.env'), {encoding: "utf-8"} )
        var reiniciar = false
        if(!process.env.API_REMOVE_BG){
          var removebg = "\n# REMOVEBG - Coloque abaixo sua chave API do site remove.bg (REMOVER FUNDO DE IMAGENS)\n"+
          "API_REMOVE_BG=??????\n\n"
          env += removebg
          reiniciar = true
        }
        if(!process.env.API_NEWS_ORG){
          var newsapi = "\n# NEWSAPI- Coloque abaixo sua chave API do site newsapi.org (NOTICIAIS ATUAIS)\n"+
          "API_NEWS_ORG=??????\n\n"
          env += newsapi
          reiniciar = true
        }
        if(!process.env.API_RAPIDAPI){
          var rapidapi = "\n# RAPIDAPI - Coloque sua chave API do site rapidapi.com (PESQUISA IMAGENS/WEB/ETC)\n"+
          "API_RAPIDAPI=??????\n\n"
          env += rapidapi
          reiniciar = true
    ...
    verificarEnv:()=>{
        ...
        let resposta = {
          ...
          console.log("[ENV]", corTexto(resposta.removebg.resposta, resposta.removebg.cor_resposta))
          console.log("[ENV]", corTexto(resposta.newsapi.resposta, resposta.newsapi.cor_resposta))
          console.log("[ENV]", corTexto(resposta.rapidapi.resposta, resposta.rapidapi.cor_resposta))

...
const { consoleErro, erroComandoMsg, obterNomeAleatorio } = require('./util');
    ...
    obterYtMp3: async (video)=>{ //OBTEM UM VIDEO DO YOUTUBE(MP3)
        try{
            ...
            const downloaderYT = new DownloadYTFile({ 
                outputPath: path.resolve("media/audios/baixados"),
             ...
             await downloaderYT.download(video.id,nomeArquivo)
             downloaderYT.removeAllListeners()
             return path.resolve(`media/audios/baixados/${nomeArquivo}`)

...
const criarArquivosNecessarios = async ()=>{
    try {
      ...
      if(!fs.existsSync(path.resolve('.env'))) {
        ...
        "API_SCALE_SERP=??????\n"+
        "# Coloque sua chave API do site avesapi.com (PESQUISA WEB GOOGLE)\n"+
        "API_AVESAPI=??????\n"+
...
const verificarConfigEnv = ()=>{
    let resposta = {
      ...
      avesapi : {
        resposta: (process.env.API_AVESAPI == "??????") ? "A API do AVESAPI ainda não foi configurada" : "✓ API AVESAPI Configurada.",
        cor_resposta: (process.env.API_AVESAPI == "??????") ? "red" : false
      },
      twitter :{
    ...
    console.log("[ENV]", color(resposta.avesapi.resposta, resposta.avesapi.cor_resposta))

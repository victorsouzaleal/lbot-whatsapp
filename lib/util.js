...
module.exports = {
    ...
    criarArquivosNecessarios : async ()=>{
        try {
          ...
          const existePastaJson = fs.pathExistsSync(path.resolve("database/json")), existePastaDb = fs.pathExistsSync(path.resolve("database/db"))
          ...
          if(!existePastaDb) fs.mkdirSync(path.resolve("database/db"), {recursive: true})
          if(!existePastaJson) fs.mkdirSync(path.resolve("database/json"), {recursive: true})

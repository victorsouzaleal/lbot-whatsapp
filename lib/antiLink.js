...
module.exports = antiLink = async (client, message) => {
    try{
        const {sender, from, isGroupMsg, body, chat, type, caption } = message
        if(isGroupMsg){
                ...
                if (!isBotGroupAdmins) {
                    ...
                    if(body != undefined){
                        ...
                        if((links_permitidos_qtd != links_gerais) && (!groupAdmins.includes(sender.id))){
                            ...
                            await client.sendTextWithMentions(from, criarTexto(msgs_texto.geral.resposta_ban, sender.id.replace("@c.us", ""), msgs_texto.grupo.antilink.motivo, botInfo().nome))
                            return
                        }

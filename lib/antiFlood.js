            ...
            if(afl_status.antiflood){
                ...
                if(flood) {
                    if(!groupAdmins.includes(sender.id)) {
                        client.removeParticipant(groupId, sender.id).then(async ()=>{
                            await client.sendTextWithMentions(from, criarTexto(msgs_texto.geral.resposta_ban, sender.id.replace("@c.us", ""), msgs_texto.grupo.antiflood.motivo, botInfo().nome))          
                        })
                    }
                } 
            }

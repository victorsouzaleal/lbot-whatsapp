...
export const imagemUpload = async (bufferImagem) =>{
    try{
        ...
        await service.uploadFromBinary(bufferImagem, obterNomeAleatorio(".png")).then(({directLink})=>{
            resposta = {sucesso: true, link: directLink}
        }).catch(()=>{
            resposta = {sucesso: false, erro: `erro_upload`}
        })
        return resposta
    } catch(err){
        throw new Error("Falha na função imagemUpload")
    }
}
...
export const misturarEmojis = (emoji1, emoji2)=>{
    try{
        let resposta = {sucesso: false}
        try{
            let emojiUrl = getEmojiMixUrl(emoji1, emoji2)
            if(emojiUrl != undefined) {
                resposta = {sucesso: true, url: emojiUrl}
            } else {
                resposta = {sucesso: false, erro: "Emojis não compatíveis"}
            }
        } catch{
            resposta = {sucesso: false, erro: "Emojis não compatíveis"}
        }
        ...
        throw new Error("Falha na função misturarEmojis")
    }
}

export const respostaHercaiTexto = async(textoUsuario)=>{
    try{
        let resposta = {sucesso: false}
        const herc = new Hercai()
        await herc.question({model:'v3', content: textoUsuario}).then((respostaHercai)=>{
...
export const respostaHercaiImagem= async(textoUsuario)=>{
    try{
        let resposta = {sucesso: false}
        const herc = new Hercai()
        let {traducao} = await obterTraducao(textoUsuario, 'en')
        await herc.betaDrawImage({prompt: traducao, width: 256, height:256}).then((respostaHercai)=>{
            if(respostaHercai.status == 404) resposta = {sucesso: false, erro: 'O texto que você colocou é inválido ou não pode ser criado.'}
            else resposta = {sucesso: true, url: respostaHercai.url}
        }).catch((err)=>{
            if(err.response?.status == 429) {
                resposta = {sucesso: false, erro:'Limite de pedidos foi excedido, tente novamente mais tarde'}
            } else{
...
export const removerFundo = async(imagemBuffer)=>{
    try{
        ...
        let respostaUpload = await axios.request(config).catch((err)=>{
        ...
        let imagemBufferResposta = await axios.get(fotoUrl[0], {responseType: 'arraybuffer'}).catch(()=>{
            resposta = {sucesso: false, erro: "Erro ao baixar a imagem sem fundo"}
            return resposta
        })
        return {sucesso: true, imagemBuffer: imagemBufferResposta.data}
    } catch(err){
...
export const obterNoticias = async ()=>{
    try {
        let resposta = []
        let listaNoticias = await google.getTopNews('pt')
...
export const obterImagens = async (pesquisaTexto)=>{ 
    try {
        let imagens = await google.image(pesquisaTexto, { safe: false, additional_params:{hl: 'pt'}})

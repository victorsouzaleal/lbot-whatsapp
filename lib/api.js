...
module.exports = {
    ...
    obterMidiaFacebook : async(url)=>{
        try {
            try{
                var res = await axios.get(`https://docs-jojo.herokuapp.com/api/fb?url=${url}`)
                var videoDuracao = await getVideoDurationInSeconds(res.data.result.hd || res.data.result.normal)
                ...
                var config = {
                    method: 'post',
                    url: 'https://downloaderfb.com/service.php',
                    headers: { 
                        'Content-Type': 'application/x-www-form-urlencoded', 
                        'Cookie': '__cfduid=d9a0dc3ef56ad1c55cfbd70b4f5a385811616719575'
                    },
                    data : data
                };

                var res = await axios(config)
                var videoDuracao = await getVideoDurationInSeconds(res.data.hd_download_url || res.data.sd_download_url)
                if(res.data.type= "success"){
                    return {
                        encontrado : true,
                        duracao : videoDuracao,
                        url: res.data.hd_download_url|| res.data.sd_download_url
                    }
                } else {
                    return {encontrado: false}
                }
    ...
    obterYTMp3: async (videoInfo)=>{
        try{
             var nomeArquivo = obterNomeAleatorio(".mp3")
             downloaderYT.on("error", (err)=>{
                downloaderYT.removeAllListeners()
                throw err
            })
             await downloaderYT.download(videoInfo.id, nomeArquivo)
             downloaderYT.removeAllListeners()
             return path.resolve(`media/audios/${nomeArquivo}`)
        } catch(err) {
            consoleErro(err.message, "API obterYTMp3")
            throw new Error(msgs_texto.downloads.play.erro_download)
        }
    },
    ...
    obterTabelaNick: async()=>{
        const githubGistTabela = await axios.get("https://gist.githubusercontent.com/victorsouzaleal/9a58a572233167587e11683aa3544c8a/raw/dc13fa19b323ae2ee2e694d8fccf25970bec04a2/tabela.txt")

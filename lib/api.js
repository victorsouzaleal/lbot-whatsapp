...
const {obterDuracaoVideo} = require('./conversao')
...
module.exports = {
    ...
    obterMidiaFacebook : async(url)=>{
        try {
            ...
            if(res.data.result.hd || res.data.result.normal){
                var videoDuracao = await obterDuracaoVideo(res.data.result.hd || res.data.result.normal)
                return {
                    encontrado : true,

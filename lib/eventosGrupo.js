...
module.exports = eventosGrupo = async (client, event) => {
    try {
        ...
        if (event.action == 'add') {
            ...
            let existeParticipante = await participanteExiste(event)
            if(existeParticipante) return
            //SE O PARTICIPANTE ESTIVER NA LISTA NEGRA, EXPULSE
            let estaListado = await verificarUsuarioListaNegra(client,event)
            if(estaListado) return
            //ANTIFAKE
            let antiFakeLiberado = await antiFake(client,event,g_info)
            if(!antiFakeLiberado) return
            //BEM-VINDO

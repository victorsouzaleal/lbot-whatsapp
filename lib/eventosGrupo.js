...
const {atualizarParticipantesAdd, atualizarParticipantesRemover} = require("./atualizarParticipantes")
...
module.exports = eventosGrupo = async (client, event) => {
    try {
        ...
        if (event.action == 'add') {
            ...
            await atualizarParticipantesAdd(event.chat,event.who)

        } else if(event.action == "remove"){
            //ATUALIZA A LISTA DE PARTICIPANTES NO BANDO DE DADOS
            await atualizarParticipantesRemover(event.chat,event.who)

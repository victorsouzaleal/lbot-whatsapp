...
module.exports = chamadaComando = async (c, messageTranslated) => {
    try {
        const {sender, isGroupMsg, chat, type, caption, id, chatId, body, username} = messageTranslated, formattedTitle = chat?.formattedTitle || null
        const t = moment.now()
        ...
        const isGroupAdmins = isGroupMsg ? chat.isAdmin : false
        ...
        const abrirMenu = commands.match(/comandos|comando|ajuda|menu|help/gmi) && !isGroupMsg && !comandoExiste && !messageTranslated.fromMe
        ...
        if(comandoExiste){
            if(lista_comandos.utilidades.includes(command)){
                //UTILIDADES
                if(msgGuia) return await client.reply(c,chatId, guiaComandoMsg("utilidade", command), id)
                ...
                //FIGURINHAS
                if(msgGuia) return await client.reply(c,chatId, guiaComandoMsg("figurinhas", command), id)
                ...
                //DOWNLOADS
                if(msgGuia) return await client.reply(c, chatId, guiaComandoMsg("downloads", command), id)
                ...
                //GRUPO
                if(msgGuia) return await client.reply(c, chatId, guiaComandoMsg("grupo", command), id)
                ...
                //DIVERSÃO
                if(msgGuia) return await client.reply(c, chatId, guiaComandoMsg("diversao", command), id)
                ...
                //ADMIN
                if(msgGuia) return await client.reply(c, chatId, guiaComandoMsg("admin", command), id)
                ...
                //INFO
                if(msgGuia) return await client.reply(c, chatId, guiaComandoMsg("info", command), id)
                await info(c, abrirMenu, messageTranslated)
            ...
            if(isGroupMsg && (type == MessageTypes.image || type == MessageTypes.video) && grupoInfo.autosticker){
                ...
                if(botInfo().limitecomandos.status){
                    ...
                    if(limiteComando.comando_bloqueado) if(limiteComando.msg != undefined) return await client.reply(c,chatId, limiteComando.msg, id)
                }
                ...
                if(botInfo().limite_diario.status){
                    ...
                    else return await client.reply(c, chatId, criarTexto(msgs_texto.admin.limitediario.resposta_excedeu_limite, username, ownerNumber), id)
                } else {
            ...
            if(!isGroupMsg && botInfo().limitarmensagens.status){
                ...
                if(limitarMensagens.bloquear_usuario) {
                    await client.sendText(sender, limitarMensagens.msg)
                    return await client.contactBlock(sender)
                }
            }
        }
    } catch (err) {

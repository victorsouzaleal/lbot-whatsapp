...
export const chamadaComando = async (c, messageTranslated) => {
    try {
        const lista_comandos = listarComandos(), msgs_texto = obterMensagensTexto(), botInfoJSON = botInfo(), {prefixo, nome_bot, nome_adm, numero_bot} = botInfoJSON
        const {sender, isGroupMsg, type, caption, id, chatId, body, username} = messageTranslated
        const t = moment.now()
        const commands = caption || body || ''
        var command = commands.toLowerCase().split(' ')[0] || ''
        const args =  commands.split(' ')
        const ownerNumber = numero_bot

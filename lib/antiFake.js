...
module.exports = antiFake = async (c,event,g_info) =>{
    try{
        if(g_info.antifake.status){
            let participantId = event.participants[0], botNumber = await client.getHostNumber(c),  groupAdmins = await client.getGroupAdmins(event.id), isBotGroupAdmins = groupAdmins.includes(botNumber)
            if(!isBotGroupAdmins){

...
export const menuPrincipal  = ()=> { 
    let {nome_bot, nome_adm, prefixo} = obterBotVariaveis()
    return `__| ☾ *🤖 ${nome_bot?.trim()}®* ☽ 
|
|>---- ☾ 🤖 *MENU PRINCIPAL* 🤖☽
|
...
|- *${prefixo}menu* 1 -> Figurinhas
|- *${prefixo}menu* 2 -> Utilidades
|- *${prefixo}menu* 3 -> Downloads
|- *${prefixo}menu* 4 -> Grupo
|- *${prefixo}menu* 5 -> Diversão
|- *${prefixo}menu* 6 -> Créditos
|
...
|>---- ☾ 📥 *DOWNLOADS* 📥☽
|
|- *${prefixo}play* [nome-musica] - Faz download de uma música e envia.
|- *${prefixo}yt* [nome-video] - Faz download de um video do Youtube e envia.
|- *${prefixo}fb* [link-post] - Faz download de um video do Facebook e envia.
|- *${prefixo}ig* [link-post] - Faz download de um video/foto do Instagram e envia.
|- *${prefixo}tw* [link-tweet] - Faz download de um video/foto do Twitter e envia.
|- *${prefixo}tk* [link-tiktok] - Faz download de um video do Tiktok e envia.
|- *${prefixo}img* [tema-imagem] - Faz download de uma imagem e envia.
|
╰╼❥ ${nome_bot?.trim()}® by *${nome_adm?.trim()}*`
}
...
|>---- ☾ ⚒️ *UTILITÁRIOS* ⚒️☽
|
|- *${prefixo}voz* [idioma] [mensagem] - Transforma texto em audio.
|- *${prefixo}ouvir* - Responda um áudio para transformar em texto.
|- *${prefixo}audio* [tipo-edicao] - Responda um audio com este comando para fazer a edição.
|- *${prefixo}qualmusica* - Responda um audio/video para identificar a música.
|- *${prefixo}letra* [nome-musica] - Envia a letra da música que você digitar.
|- *${prefixo}anime* -  Identifica o anime por foto de uma cena.
|- *${prefixo}tabela* -  Mostra tabela com letras para criação de nicks.
|- *${prefixo}traduz* [idioma] [texto] - Traduz um texto para o idioma escolhido.
|- *${prefixo}ddd* - Responda alguém para ver o estado/região.
|- *${prefixo}pesquisa* [tema] - Faz uma rápida pesquisa na internet.
|- *${prefixo}clima* [cidade] - Mostra a temperatura atual.
|- *${prefixo}noticias* - Obtem noticias atuais.
|- *${prefixo}moeda* [real|dolar|euro] [valor] - Converte o valor de uma determinada moeda para outras.
|- *${prefixo}calc* [expressão-matemática] - Calcula alguma conta que queira fazer.
|- *${prefixo}rastreio* [código-rastreio] - Rastreamento dos CORREIOS.
|
╰╼❥ ${nome_bot?.trim()}® by *${nome_adm?.trim()}*`
}

export const menuGrupo = (isGroupAdmin)=>{
    let {nome_bot, nome_adm, prefixo} = obterBotVariaveis()
    if(isGroupAdmin){
        return `__| ☾ *🤖 ${nome_bot?.trim()}®* ☽ 
|
...
export const menuDiversao = (isGroup)=>{
    let {nome_bot, nome_adm, prefixo} = obterBotVariaveis()
    if(isGroup){
        return `__| ☾ *🤖 ${nome_bot?.trim()}®* ❣ 
|
...
export const menuCreditos = ()=>{
    let {nome_bot, nome_adm, prefixo} = obterBotVariaveis()
    return `☾ *🤖 ${nome_bot?.trim()}®* ☽ 
...
|- *${prefixo}limitarmsgs* [qtd-msgs] [intervalo] - Ativa/desativa o limite de mensagens privadas em um intervalo.

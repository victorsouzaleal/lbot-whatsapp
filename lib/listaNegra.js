...
export const verificacaoListaNegraGeral = async(c, groupsInfo)=>{
    try {
        ...
        for(var grupo of groupsInfo){
            ...
            if(isBotGroupAdmins){
                let groupId = grupo.id, participantesGrupo = await socket.getGroupMembersIdFromMetadata(grupo), lista_negra = await gruposdb.obterListaNegra(groupId), usuarios_listados = []
...
export const verificarUsuarioListaNegra = async(c, event)=>{
    try{
        var msgs_texto = await obterMensagensTexto()
        let botInfo = await bot.obterInformacoesBot()
        const botNumber = await bot.obterNumeroBot(), groupAdmins = await grupos.obterAdminsGrupo(event.id),  isBotGroupAdmins = groupAdmins.includes(botNumber)
        if(isBotGroupAdmins){
            let lista_negra = await gruposdb.obterListaNegra(event.id)

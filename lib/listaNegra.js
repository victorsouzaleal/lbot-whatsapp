...
module.exports = {
    verificacaoListaNegraGeral : async(c, groupsInfo)=>{
        try {
            for(var grupo of groupsInfo){
                var botNumber = await client.getHostNumberFromBotJSON(), groupAdmins = await client.getGroupAdminsFromMetadata(grupo),  isBotGroupAdmins = groupAdmins.includes(botNumber)
                if(isBotGroupAdmins){
                    let groupId = grupo.id, participantesGrupo = await client.getGroupMembersIdFromMetadata(grupo), lista_negra = await db.obterListaNegra(groupId), usuarios_listados = []
    ...
    verificarUsuarioListaNegra: async(c, event)=>{
        try{
            const botNumber = await client.getHostNumberFromBotJSON(), groupAdmins = await client.getGroupAdminsFromDb(event.id),  isBotGroupAdmins = groupAdmins.includes(botNumber)
            if(isBotGroupAdmins){

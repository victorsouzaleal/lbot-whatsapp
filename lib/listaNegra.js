...
module.exports = {
    verificacaoListaNegraGeral : async(c, groupsInfo)=>{
        try {
            for(var grupo of groupsInfo){
                var botNumber = await socket.getHostNumberFromBotJSON(), groupAdmins = await socket.getGroupAdminsFromMetadata(grupo),  isBotGroupAdmins = groupAdmins.includes(botNumber)
                if(isBotGroupAdmins){
    ...
    verificarUsuarioListaNegra: async(c, event)=>{
        try{
            const botNumber = await socket.getHostNumberFromBotJSON(), groupAdmins = await socket.getGroupAdminsFromDb(event.id),  isBotGroupAdmins = groupAdmins.includes(botNumber)
            if(isBotGroupAdmins){

...
const {timestampParaData} = require("./util")
...
module.exports = {
    ...
    botVerificarCampos : async ()=>{
        let bot = JSON.parse(fs.readFileSync(path.resolve('database/json/bot.json')))
        if(!bot.nome) bot.nome = "LBOT"
        if(!bot.criador) bot.criador = "Leal"
        if(!bot.iniciado) bot.iniciado = 0
        if(!bot.cmds_executados) bot.cmds_executados = 0
        if(!bot.autosticker) bot.autosticker = false
        if(!bot.bloqueio_cmds) bot.bloqueio_cmds = []
        if(!bot.antitrava) {
            bot.antitrava = {
                status: false,
                max_caracteres: 0
        ...
        if(!bot.limitarmensagens){
            bot.limitarmensagens = {
                status:false,
                max: 10,
                intervalo: 10,
                msgs:[]

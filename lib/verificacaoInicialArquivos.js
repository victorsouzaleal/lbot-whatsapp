...
export const criarArquivosNecessarios = async ()=>{
  try {
    const existePastaDados = fs.pathExistsSync(path.resolve("database/dados_salvos"))
    const existePastaTemp = fs.pathExistsSync(path.resolve("temp"))
    const existeBotJson = fs.existsSync(path.resolve("database/dados_salvos/bot.json")), existeEnv = fs.existsSync(path.resolve('.env'))

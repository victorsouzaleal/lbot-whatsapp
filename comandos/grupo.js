    ...
    try{
        ...
        switch(command){
            ...
            case '!status':
                ...
                status_resposta += (g_status.antilink.status) ? criarTexto(msgs_texto.grupo.status.resposta_variavel.antilink.on, al_filtros) : msgs_texto.grupo.status.resposta_variavel.antilink.off
                //Anti-fake
                status_resposta += (g_status.antifake.status) ? criarTexto(msgs_texto.grupo.status.resposta_variavel.antifake.on, g_status.antifake.ddi_liberados.toString()) : msgs_texto.grupo.status.resposta_variavel.antifake.off
                //Anti-flood
                let infoAntiFlood = db.grupoInfoAntiFlood(groupId)
                status_resposta += (g_status.antiflood) ? criarTexto(msgs_texto.grupo.status.resposta_variavel.antiflood.on, infoAntiFlood.max, infoAntiFlood.intervalo) : msgs_texto.grupo.status.resposta_variavel.antiflood.off 
                //Contador
                status_resposta += (g_status.contador.status) ? criarTexto(msgs_texto.grupo.status.resposta_variavel.contador.on, g_status.contador.inicio) : msgs_texto.grupo.status.resposta_variavel.contador.off
                //Bloqueio de CMDS
                status_resposta += (g_status.block_cmds.length != 0) ? criarTexto(msgs_texto.grupo.status.resposta_variavel.bloqueiocmds.on, g_status.block_cmds.toString()) : msgs_texto.grupo.status.resposta_variavel.bloqueiocmds.off
                //Lista Negra
                status_resposta += criarTexto(msgs_texto.grupo.status.resposta_variavel.listanegra, g_status.lista_negra.length)
            ...
            case "!atividade":
                ...
                let atividade_resposta = criarTexto(msgs_texto.grupo.atividade.resposta, atividade_usuario.msg, atividade_usuario.texto, atividade_usuario.imagem, atividade_usuario.video, atividade_usuario.sticker, atividade_usuario.gravacao, atividade_usuario.audio, atividade_usuario.outro)
            ...
            case "!imarcar":
                ...
                if(qtd_inativos > 0){
                    let imarcar_resposta = criarTexto(msgs_texto.grupo.minativos.resposta_titulo, args[1], qtd_inativos)
                    imarcar_resposta += `═════════════════\n`
                    for(let usuario of mi_usuarios){
                        if(usuario.id_usuario != botNumber+"@c.us") imarcar_resposta += criarTexto(msgs_texto.grupo.minativos.resposta_itens, usuario.id_usuario.replace(/@c.us/g, ''), usuario.msg)
                    }
            ...
            case "!ibanir":
                ...
                if(bi_usuarios.length != 0){
                    ...
                    client.reply(from,criarTexto(msgs_texto.grupo.binativos.sucesso, bi_usuarios.length - 1, args[1]),id)
                } else {
            ...
            case "!topativos":
                ...
                let topativos_resposta = criarTexto(msgs_texto.grupo.topativos.resposta_titulo, args[1])
                for (let i = 0 ; i < top_ativos.length ; i++){
                    ...
                    topativos_resposta += criarTexto(msgs_texto.grupo.topativos.resposta_itens, medalha, i+1, top_ativos[i].id_usuario.replace(/@c.us/g, ''), top_ativos[i].msg)   
                }
                topativos_resposta += '╠\n╚═〘 LBOT® 〙'
            ...
            case "!enquete":
                ...
                if(args[1] != "off"){
                    ...
                    let enquete_resultado = criarTexto(msgs_texto.grupo.enquete.resultado_titulo, pergunta)
                    for(let opcao of opcoes){
                        enquete_resultado += criarTexto(msgs_texto.grupo.enquete.resultado_itens, opcao.digito, opcao.opcao.trim(), opcao.qtd_votos)
                    }
            ...
            case '!verenquete':
                ...
                let verenquete_resposta = criarTexto(msgs_texto.grupo.verenquete.resposta_titulo, ver_grupo.enquete.pergunta)
                for(let opcao of ver_grupo.enquete.opcoes){
                    verenquete_resposta += criarTexto(msgs_texto.grupo.verenquete.resposta_itens, opcao.digito, opcao.opcao.trim())
                }
            ...
            case "!votacao":
                ...
                if(!vb_status.voteban.status) {
                    ...
                    client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.voteban.votacao_resposta, vb_status.voteban.usuario))
                }
                break
            ...
            case '!votar':
                ...
                await client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.voteban.voto_sucesso, sender.id, votar_status.voteban.usuario.replace("@c.us",""), eval(votar_status.voteban.votos + 1), votar_status.voteban.max))
                if(voteban){
                    if (isBotGroupAdmins) {
                        await client.removeParticipant(from, votar_status.voteban.usuario)
                        .then(()=>{
                            client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.voteban.ban_resposta, votar_status.voteban.usuario))
                        }).catch(()=>{
            ...
            case "!vb":
                ...
                if(args[1] == "on"){
                    ...
                    client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.voteban.votacao_aberta_resposta, mentionedJidList[0], args[3]))
                } else if(args[1] == "off"){
                    if (!vtb_status.voteban.status) return client.reply(from,msgs_texto.grupo.voteban.sem_votacao,id)
                    client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.voteban.votacao_encerrada_resposta, vtb_status.voteban.usuario))
                    await db.alterarVoteban(groupId,false)
                } else {
                    client.reply(from,erroComandoMsg(command),id)
                }
                ...
                for(let b_cmd of b_cmd_inseridos){
                    if(lista_comandos.utilidades.includes(b_cmd) || lista_comandos.diversao.includes(b_cmd)){
                        if(b_cmd_grupo.block_cmds.includes(b_cmd)){
                            bcmd_resposta += criarTexto(msgs_texto.grupo.bcmd.resposta_variavel.ja_bloqueado, b_cmd)
                        } else {
                            b_cmd_verificados.push(b_cmd)
                            bcmd_resposta += criarTexto(msgs_texto.grupo.bcmd.resposta_variavel.bloqueado_sucesso, b_cmd)
                        }
                    } else if (lista_comandos.admin_grupo.includes(b_cmd) || lista_comandos.admin.includes(b_cmd) ){
                        bcmd_resposta += criarTexto(msgs_texto.grupo.bcmd.resposta_variavel.comando_admin, b_cmd)
                    } else {
                        bcmd_resposta += criarTexto(msgs_texto.grupo.bcmd.resposta_variavel.nao_existe, b_cmd)
                    }
                }
            ...
            case "!dcmd":
                ...
                for(let d_cmd of d_cmd_inseridos){
                    if(d_cmd_grupo.block_cmds.includes(d_cmd)) {
                        ...
                        dcmd_resposta += criarTexto(msgs_texto.grupo.dcmd.resposta_variavel.desbloqueado_sucesso, d_cmd)
                    } else {
                        dcmd_resposta += criarTexto(msgs_texto.grupo.dcmd.resposta_variavel.ja_desbloqueado, d_cmd)
                    }
                }
            ...
            case '!link':
                ...
                client.sendLinkWithAutoPreview(from, inviteLink, criarTexto(msgs_texto.grupo.link.resposta, name))
                break

            case '!adms':
                let adms_resposta = msgs_texto.grupo.adms.resposta_titulo
                for (let adm of groupAdmins) {
                    adms_resposta += criarTexto(msgs_texto.grupo.adms.resposta_itens, adm.replace(/@c.us/g, ''))
                }
            ...
            case '!mt':
                ...
                var userInputText = body.slice(4).trim()
                var response = (args.length > 1) ? criarTexto(msgs_texto.grupo.mt.resposta_titulo_variavel, userInputText) : msgs_texto.grupo.mt.resposta_titulo_comum
                for (let i = 0; i < groupMembers.length; i++) {
                    response += criarTexto(msgs_texto.grupo.mt.resposta_itens, groupMembers[i].id.split("@")[0])
                }
                response += '╚═〘 LBOT®〙'
                await client.sendTextWithMentions(from, response)
                break       
            ...
            case '!promover':
                ...
                await client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.promover.sucesso, mentionedJidList[0]))
                break
            ...
            case '!rebaixar':
                ...
                await client.sendTextWithMentions(from, criarTexto(msgs_texto.grupo.rebaixar.sucesso, mentionedJidList[0]))
                break

...
const {makeText, firstUpperCase, errorCommandMsg} = require("../lib/util")
...
module.exports = diversao = async(client,message) => {
    try {
        ...
        switch(command){
            ...
            case '!viadometro' :
                if (!isGroupMsg) return client.reply(from, msgs_texto.permissao.grupo, id)
                if(!quotedMsg && mentionedJidList.length == 0) return client.reply(from, errorCommandMsg(command), id)
                ...
                let viadometro_resposta = makeText(msgs_texto.diversao.viadometro.resposta,viadometro_resps[aleatorio])
            ...
            case '!bafometro' :
                ...
                let bafometro_resposta = makeText(msgs_texto.diversao.bafometro.resposta,bafometro_resps[bafometro_aleatorio])
            ...
            case "!caracoroa":
                ...
                client.reply(from,msgs_texto.diversao.caracoroa.espera,id).then(()=>{
                    let caracoroa_resposta = makeText(msgs_texto.diversao.caracoroa.resposta, firstUpperCase(lados[lado_aleatorio]))
            ...
            case "!ppt":
                ...
                if(vitoria_usuario == true) {
                    client.reply(from, makeText(msgs_texto.diversao.ppt.resposta.vitoria, icone_escolha_usuario, icone_escolha_bot), id)
                } else if(vitoria_usuario == false){
                    client.reply(from, makeText(msgs_texto.diversao.ppt.resposta.derrota, icone_escolha_usuario, icone_escolha_bot), id)
                } else {
                    client.reply(from, makeText(msgs_texto.diversao.ppt.resposta.empate, icone_escolha_usuario, icone_escolha_bot), id)
                }
            ...
            case '!roletarussa':
                ...
                let roleta_resposta = makeText(msgs_texto.diversao.roletarussa.resposta,membros_id[membro_id_index].replace(/@c.us/g, ''))
            ...
            case '!casal':
                ...
                let casal_resposta = makeText(msgs_texto.diversao.casal.resposta, pessoa1.replace(/@c.us/g, ''), pessoa2.replace(/@c.us/g, ''))
            ...
            case '!gadometro':
                ...
                let gadometro_resposta = makeText(msgs_texto.diversao.gadometro.resposta, gadometro_resps[gado_aleatorio])
            ...
            case '!top5':
                ...
                let top5_resposta = makeText(msgs_texto.diversao.top5.resposta_titulo, tema_ranking)
                for (let i = 0 ; i < 5 ; i++){
                    let medalha = ""
                    switch(i+1){
                        case 1:
                    ...
                    top5_resposta += makeText(msgs_texto.diversao.top5.resposta_itens, medalha, i+1, m_atual.replace(/@c.us/g, ''))
            ...
            case '!par':
                if (!isGroupMsg) return client.reply(from, msgs_texto.permissao.grupo, id)
                if(mentionedJidList.length !== 2) return client.reply(from, errorCommandMsg(command) , id)
                const par_resps = msgs_texto.diversao.par.respostas
                let par_aleatorio = Math.floor(Math.random() * par_resps.length)
                let par_resposta = makeText(msgs_texto.diversao.par.resposta, mentionedJidList[0].replace(/@c.us/g, ''), mentionedJidList[1].replace(/@c.us/g, ''), par_resps[par_aleatorio])
                client.sendTextWithMentions(from, par_resposta)
                break

            case "!fch":
                try{
                    var response = await api.getCardsAgainstHu()

...
export class BotService {
    ...
    // Antispam
    public setAntispamCommands(status: boolean, maxCommandsMinute: number, blockTime: number){
        let bot = this.getBot()
    ...
    public isSpamCommand(idUser: string, isAdminBot: boolean, isAdminGroup: boolean){
        let bot = this.getBot()
        ...
        if(!isAdminBot && !isAdminGroup){
            ...
            let limitedUserIndex = bot.antispam_cmds.limited_users.findIndex(usuario => usuario.id_user == idUser)
            //SE O USUÁRIO NÃO ESTIVER LIMITADO
            if(limitedUserIndex === -1){
                //OBTEM O INDICE DO USUARIO NA LISTA DE USUARIOS
                let userIndex = bot.antispam_cmds.users.findIndex(user=> user.id_user == idUser)
                ...
                if(userIndex !== -1){
                    ...
                    if(bot.antispam_cmds.users[userIndex].cmds >= bot.antispam_cmds.max_cmds_minute){ //SE ATINGIR A QUANTIDADE MAXIMA DE COMANDOS POR MINUTO
                        ...
                        bot.antispam_cmds.limited_users.push({id_user: idUser, expiration: CURRENT_TIMESTAMP + bot.antispam_cmds.block_time})
                        bot.antispam_cmds.users.splice(userIndex, 1)
                        isSpam = true
                    }
                } else {//SE NÃO EXISTIR NA LISTA
                    bot.antispam_cmds.users.push({id_user: idUser, cmds: 1, expiration: CURRENT_TIMESTAMP+60})
                }
            }
        }

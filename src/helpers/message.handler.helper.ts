...
export async function handlePrivateMessage(client: WASocket, botInfo: Bot, message: Message){
    ...
    if (isCommand || isAutosticker){
        ...
        if (await procs.isUserLimitedByCommandRate(client, botInfo, message)) return false

        //Se o comando estiver bloqueado globalmente, retorne.
        if (await procs.isCommandBlockedGlobally(client, botInfo, message)) return false
...
export async function handleGroupMessage(client: WASocket, group: Group, botInfo: Bot, message: Message){
    ...
    if (await procs.isBotLimitedByGroupRestricted(group, botInfo)) return false

    //Se o antilink estiver ativado, e for detectado um link na mensagem, retorne.
    if (await procs.isDetectedByAntiLink(client, botInfo, group, message)) return false

    //Se o Anti-FLOOD estiver ativado, e for detectada como FLOOD, retorne.
    if (await procs.isDetectedByAntiFlood(client, botInfo, group, message)) return false
    ...
    if (procs.isIgnoredByGroupMuted(group, message)) return false
    ...
    if (isCommand || isAutosticker){
        ...
        if (await procs.isCommandBlockedGroup(client, group, botInfo, message)) return false
